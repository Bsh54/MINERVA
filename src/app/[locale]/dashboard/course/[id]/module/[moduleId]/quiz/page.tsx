...
export default function ModuleQuizPage() {
  ...
  let moduleTitle = '';
  if (course) {
    const module = course.coursePlan.modules.find(m => m.id.toString() === moduleId);
    if (module) {
  ...
  const loadQuiz = async () => {
    ...
    try {
      const response = await fetch(`/api/courses/${courseId}/quiz`, {
        ...
        body: JSON.stringify({ targetId: moduleId, targetType: 'module' })
      });
  ...
  const handleSubmit = () => {
    ...
    saveQuizScore(moduleId, 'module', finalScore);
  };
